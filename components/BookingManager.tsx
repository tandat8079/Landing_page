...
export function BookingManager() {
  ...
  const [mode, setMode] = useState<"form" | "success" | "detail" | "cancel">(
    "form",
  );
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("hotel:book", openBooking);
      window.removeEventListener("hotel:dates", dates);
    };
  }, []);
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!/^[\p{L}\s.'’-]{2,}$/u.test(fullName))
      nextFieldErrors.fullName =
        "Họ tên chỉ được chứa chữ, không được chứa số.";
    if (!/^0\d{9}$/.test(phone))
    ...
    if (Object.keys(nextFieldErrors).length) {
      ...
      requestAnimationFrame(() =>
        document
          .getElementById(
            nextFieldErrors.fullName ? "booking-full-name" : "booking-phone",
          )
          ?.focus(),
      );
      return;
    ...
    const newBooking: Booking = {
      code: `AN${Date.now().toString().slice(-6)}`,
      roomIndex,
      checkIn,
      checkOut,
      guests,
  ...
  return (
    <>
      {booking && (
        <button className="my-booking" onClick={showDetail}>
          <span>✓</span>
          <span>
            Đặt phòng của tôi<small>{booking.code}</small>
          </span>
        </button>
      )}
      {open && (
        ...
        >
          ...
          >
            ...
            {mode === "form" && (
              <>
                <div className="booking-panel__hero">
                  <Image src={room.image} alt={room.name} fill sizes="480px" />
                  <span>ĐẶT PHÒNG TRỰC TIẾP</span>
                </div>
                <form className="booking-flow" onSubmit={submit}>
                  <p className="booking-flow__step">BƯỚC 01 / 02</p>
                  <h2 id="booking-panel-title">Hoàn tất kỳ nghỉ</h2>
                  <p className="booking-flow__room">
                    {room.name} · {room.meta}
                  </p>
                  {error && (
                    <p
                      className="booking-flow__error"
                      role="alert"
                      tabIndex={-1}
                    >
                      {error}
                  ...
                  <fieldset>
                    <legend>Thông tin khách chính</legend>
                    <div className="booking-flow__grid">
                      <label>
                        Họ và tên
                        <input
                      ...
                      <label className="booking-flow__full">
                        Lời nhắn (không bắt buộc)
                        <textarea
                          name="note"
                          placeholder="Dị ứng thực phẩm, giờ đến dự kiến..."
                        />
                      </label>
                    </div>
                  </fieldset>
                  <div className="booking-summary">
                    <span>
                      {nights || 0} đêm × {room.price} VNĐ
                    </span>
                    <strong>{money.format(total)} VNĐ</strong>
                    <small>
                      Đã bao gồm thuế phí · Chưa phát sinh thanh toán trong bản
                      demo
                    </small>
                  </div>
                  <button className="booking-flow__submit" type="submit">
                    Xác nhận đặt phòng <span>→</span>
                  </button>
                </form>
              </>
            )}
            {mode === "success" && activeBooking && (
              <div className="booking-state booking-state--success">
                <span className="booking-state__icon">✓</span>
                <p>ĐẶT PHÒNG THÀNH CÔNG</p>
                <h2 id="booking-panel-title">
                  Hẹn gặp bạn
                  <br />
                  giữa đại ngàn.
                </h2>
                <p>Mã đặt phòng</p>
                <strong>{activeBooking.code}</strong>
                <p>Xác nhận demo đã được lưu trên trình duyệt này.</p>
                <button onClick={() => setMode("detail")}>
                  Xem chi tiết đặt phòng
                </button>
              </div>
            )}
            {(mode === "detail" || mode === "cancel") && activeBooking && (
              <div className="booking-detail">
                <p className="booking-flow__step">CHI TIẾT ĐẶT PHÒNG</p>
                <h2 id="booking-panel-title">{activeBooking.code}</h2>
                <span className="booking-detail__status">● Đã xác nhận</span>
                <div className="booking-detail__room">
                  <Image
                  ...
                  <div>
                    ...
                    <p>
                      {activeBooking.nights} đêm · {activeBooking.guests} khách
                    </p>
                  </div>
                </div>
                <dl>
                  <div>
                    <dt>Nhận phòng</dt>
                    <dd>
                      {prettyDate(activeBooking.checkIn)}
                      <small>Sau 14:00</small>
                    </dd>
                  </div>
                  <div>
                    <dt>Trả phòng</dt>
                    <dd>
                      {prettyDate(activeBooking.checkOut)}
                  ...
                  <div>
                    ...
                    <dd>
                      ...
                      <small>{activeBooking.phone}</small>
                    </dd>
                  </div>
                  <div>
                    <dt>Email</dt>
                    <dd>{activeBooking.email}</dd>
                  </div>
                  <div>
                    <dt>Tổng dự kiến</dt>
                    <dd>
                      <b>{money.format(activeBooking.total)} VNĐ</b>
                      <small>Thanh toán tại khu nghỉ</small>
                    </dd>
                  </div>
                </dl>
                {activeBooking.note && (
                  <p className="booking-detail__note">
                    <b>Lời nhắn:</b> {activeBooking.note}
                  </p>
                )}
                {mode === "cancel" ? (
                  <div className="cancel-box" role="alert">
                    <h3>Huỷ đặt phòng này?</h3>
                    <p>
                      Thao tác sẽ xoá đặt phòng demo khỏi trình duyệt. Bạn không
                      thể hoàn tác.
                    </p>
                    <div>
                      <button onClick={() => setMode("detail")}>
                      ...
                      <button onClick={cancelBooking}>Xác nhận huỷ</button>
                    </div>
                  </div>
                ) : (
